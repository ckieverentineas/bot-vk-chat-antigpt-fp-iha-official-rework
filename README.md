...
Шаг 1. Скачиваем с официального сайта Node.js версии 14.19 или выше по ссылке https://nodejs.org/en/ <br /> <br />
...
- Если не хотите качать базу данных, то просто выполните скрипт db.bat или db.sh и при активации бота запустите команду !база в лс с ботом для ее наполнения ответами из файлов в директории vk-chat-bot-master/book/.
...
SEARCH_TOP_LIMIT=1
# QUESTION_IMAGE_FONT="C:\Windows\Fonts\arial.ttf"
```
...
QUESTION_IMAGE_FONT - необязательный путь к .ttf шрифту для картинок в режиме обучения. Если не указан, бот попробует найти системный Arial/Segoe UI/DejaVu Sans. <br />
...
При обучении неизвестные вопросы показываются картинкой, чтобы бот не повторял потенциально опасный текст обычным сообщением. При обучении, редактировании и импорте базы кэш поиска автоматически сбрасывается. <br />
...
- !база - считывает txt/bin базы из директории book. Поддерживает новый блочный формат и старый IHA формат `Вопрос\Ответ\0`. <br />
- !конфиг - показывает текущую конфигурацию бота и версию бота <br />
- !режим - показывает runtime-тумблеры бота. Настройки сохраняются в базе данных и переживают перезапуск. <br />
- !режим ответы/лс/беседы/стена/оффлайн вкл|выкл - динамически включает или отключает функционал без перезапуска. <br />
- !скажи текст - доступна всем, рисует короткий текст ASCII-артом. Команда фильтрует ссылки, упоминания, длинные цифровые цепочки, неподдерживаемые символы и стоп-слова из blacklist. <br />
...
- !игнор беседа или !игнор чат - доступна root/админам в беседе, включает или снимает игнор всей текущей беседы. <br />
...
- !обучение - достает неизвестные вопросы, показывает их картинкой и предлагает скорректировать или добавить ответы. <br />
...
- EDUCATION Реализована возможность обучать бота, неизведанные вопросы будут добавляться в список неизвестных, а с помощью команды !обучение есть возможность дать ответы на них, и исправить ошибки в вопросах. Вопросы в обучении отправляются изображением, а не сырым текстом; <br />
...
- Для команды !база по пути vk-chat-bot-master/book/ кладем в директорию book корня проекта txt или bin файлы согласно форматам, описанным ниже.
...
~>Ответ
```
⚙ Старый IHA формат тоже поддерживается автоматически:
...
- В старом IHA формате приоритет в конце строки читается, но пока не сохраняется, потому что в текущей структуре базы данных нет отдельного поля для приоритета. <br />
- Команда !база сама определяет формат каждого файла и сообщает в конце, сколько файлов, вопросов, ответов и пропущенных строк обработано. <br />
- Файлы читаются потоково построчно, без загрузки всего файла в оперативную память. <br />
- Во время долгой загрузки промежуточный прогресс выводится в терминал примерно каждые 10 секунд, а в VK отправляется не чаще одного раза в минуту. <br />
- На время работы команды !база принудительно включается режим загрузки базы: автоответы пользователям, беседам и комментариям временно отключаются, а после завершения режим автоматически снимается. <br />
- Ручные тумблеры команды !режим сохраняются в таблице RuntimeSetting. Принудительный режим загрузки базы временный и в БД не записывается. <br />
- В директории book лежат образцы файлов для команды !база. <br />
____
